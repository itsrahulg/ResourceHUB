...
exports.createPhysicalResource = async (req, res) => {
  try {
    const newResource = new PhysicalResource(req.body);
    await newResource.save();

    // Update total posts ever created count
    ...
    res.status(201).json({ message: "Physical Resource created successfully" });
  } catch (error) {
